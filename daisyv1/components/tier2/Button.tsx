...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({
  ...
  return (
    <div className="daisy-button-container">
      <button
        ref={buttonRef}
        type={type}
        ...
        aria-describedby={tooltip ? `${testId || 'button'}-tooltip` : undefined}
        {...rest}
      >
        {/* DAISY v1 pattern: Complex loading state */}
        {loading && (
          <span className="daisy-button__spinner" aria-hidden="true">
            <svg viewBox="0 0 24 24" className="daisy-spinner">
              <circle
                cx="12"
                cy="12"
                r="10"
        ...
        {/* Button content */}
        <span className={`daisy-button__content ${loading ? 'daisy-button__content--loading' : ''}`}>
          {children}
        </span>
        ...
        {tooltip && tooltipVisible && (
          <div
            ...
            className="daisy-button__tooltip"
            role="tooltip"
            aria-hidden={!tooltipVisible}
          >
            {tooltip}
            <div className="daisy-button__tooltip-arrow" />
          </div>
        )}
      </button>
    </div>
...
export const withDaisyTheme = (WrappedButton: React.ForwardRefExoticComponent<ButtonProps & React.RefAttributes<HTMLButtonElement>>) => {
