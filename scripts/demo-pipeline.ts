...
async function runPipelineDemo(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `📦 Components Discovered: ${result.progress.stats.componentsDiscovered}`
    );
    console.log(
      `🔧 Components Parsed: ${result.progress.stats.componentsParsed}`
    );
    console.log(
      `📊 Components Analyzed: ${result.progress.stats.componentsAnalyzed}`
    );
    ...
    if (result.discovery) {
      ...
      result.discovery.components.forEach((component, index) => {
        ...
        console.log(`   📁 Path: ${component.sourcePath}`);
    ...
    if (result.success) {
      ...
      console.log(
        `   • Discovered ${result.progress.stats.componentsDiscovered} components`
      );
      console.log(
        `   • Pipeline completed in ${result.metrics.totalDuration}ms`
      );
