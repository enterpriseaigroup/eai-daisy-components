// NOTE: misalignment between the property boundary (the blue polygon) and the red marker pin
// The red marker is placed based on the address geocoding from Mapbox's API, which provides a general location for the address.
// The blue polygon (property boundary) is fetched from the NSW planning API, which uses different coordinate systems and might have more precise property boundaries.
...
const MapBox = ({ selectedAddress, propID, onLotSizeUpdate, isVisible }: MapBoxProps) => {
  ...
  useEffect(() => {
    const updateMap = async () => {
      ...
      const newMarker = new mapboxgl.Marker({ color: 'red' }).setLngLat(coords).addTo(map);
      setMarker(newMarker);
