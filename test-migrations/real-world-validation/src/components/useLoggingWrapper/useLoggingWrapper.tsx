...
import { useApiStatusStore } from '@presentation/store/useApiStatusStore';
  ...
  /**
   * BUSINESS LOGIC: prepareRequestBody
   *
   * WHY THIS EXISTS:
   * - Implements business logic requirement
   *
   * WHAT IT DOES:
   * 1. Implements prepareRequestBody logic
   * 2. Calls helper functions: getValidAccessToken, getAuthStoreValue, getProfileStoreValue, isUserLoggedIn
   * 3. Returns computed result
   *
   * WHAT IT CALLS:
   * - getValidAccessToken() - Function call
   * - getAuthStoreValue() - Function call
   * - getProfileStoreValue() - Function call
   * - isUserLoggedIn() - Function call
   *
   * WHY IT CALLS THEM:
   * - getValidAccessToken: Required functionality
   * - getAuthStoreValue: Required functionality
   * - getProfileStoreValue: Required functionality
   * - isUserLoggedIn: Required functionality
   *
   * DATA FLOW:
   * Input: Component state and props
   * Processing: Calls getValidAccessToken, getAuthStoreValue, getProfileStoreValue to process data
...
// TODO: isRefresh is not being triggered on page reload. Need to check if this is a bug in the library or if we need to handle it differently. (resolved)
  /**
   * BUSINESS LOGIC: useLoggingWrapper
   *
   * WHY THIS EXISTS:
   * - Implements business logic requirement
   *
   * WHAT IT DOES:
   * 1. Implements useLoggingWrapper logic
   * 2. Calls helper functions: useChatCard, useLocalStorage, useState, useApiStatusStore, useState, performance.getEntriesByType, useEffect, prepareRequestBody, setRequest, fetchData, useEffect, chatCard.mutate, useEffect, setLogMessage, setApiStatus, useEffect, console.error, setLogMessage, setApiStatus
   * 3. Returns computed result
   *
   * WHAT IT CALLS:
   * - useChatCard() - Function call
   * - useLocalStorage() - Function call
   * - useState() - Function call
   * - useApiStatusStore() - Function call
   * - useState() - Function call
   * - performance.getEntriesByType() - Function call
   * - useEffect() - Function call
   * - prepareRequestBody() - Function call
   * - setRequest() - Function call
   * - fetchData() - Function call
   * - useEffect() - Function call
   * - chatCard.mutate() - Function call
   * - useEffect() - Function call
   * - setLogMessage() - Function call
   * - setApiStatus() - Function call
   * - useEffect() - Function call
   ...
   *
   * WHY IT CALLS THEM:
   * - useChatCard: Required functionality
   * - useLocalStorage: Required functionality
   * - useState: Required functionality
   * - useApiStatusStore: Required functionality
   * - useState: Required functionality
   * - performance.getEntriesByType: Required functionality
   * - useEffect: Required functionality
   * - prepareRequestBody: Required functionality
   * - setRequest: State update
   * - fetchData: Data fetching
   * - useEffect: Required functionality
   * - chatCard.mutate: Required functionality
   * - useEffect: Required functionality
   * - setLogMessage: State update
   * - setApiStatus: State update
   * - useEffect: Required functionality
   ...
   * DATA FLOW:
   * Input: Component state and props
   * Processing: Calls useChatCard, useLocalStorage, useState to process data
   * Output: Computed value or side effect
   *
   */
export const useLoggingWrapper = (messageObject: ChatMessage, council: string, shouldLog: boolean) => {
    ...
    const isRefresh = !!value && (navType === 'reload' || legacyNav);
       ...
       *
       * WHAT IT DOES:
       * 1. Monitors shouldLog, messageObject, council for changes
       * 2. Executes prepareRequestBody, setRequest, fetchData functions
       ...
       *
       * DATA FLOW:
       * Input: shouldLog, messageObject, council state/props
       * Processing: Calls prepareRequestBody, setRequest, fetchData to process data
       * Output: Side effects executed, cleanup registered
       *
       * DEPENDENCIES:
       * - shouldLog: Triggers when shouldLog changes
       * - messageObject: Triggers when messageObject changes
    ...
    useEffect(() => {
        if (!shouldLog) return;
          /**
           * BUSINESS LOGIC: fetchData
           *
           * WHY THIS EXISTS:
           * - Implements business logic requirement
           *
           * WHAT IT DOES:
           * 1. Implements fetchData logic
           * 2. Calls helper functions: prepareRequestBody, setRequest
           * 3. Returns computed result
           *
           * WHAT IT CALLS:
           * - prepareRequestBody() - Function call
           ...
           * DATA FLOW:
           * Input: Component state and props
           * Processing: Calls prepareRequestBody, setRequest to process data
           * Output: Computed value or side effect
           *
           */
    ...
    // Effects on chatCard.mutateAsync when !apiStatuses?.['chat-card']?.status
       ...
       *
       * WHAT IT DOES:
       * 1. Monitors isRefresh for changes
       * 2. Executes chatCard.mutate functions
       * 3. Runs side effect logic
       *
       * WHAT IT CALLS:
       * - chatCard.mutate() - Function call
       ...
       *
       * DATA FLOW:
       * Input: isRefresh state/props
       * Processing: Calls chatCard.mutate to process data
       * Output: Side effects executed, cleanup registered
       *
       * DEPENDENCIES:
       * - isRefresh: Triggers when isRefresh changes
       *
       */
    useEffect(() => {
    ...
    // Effects on setLogMessage and SetApiStatus when chatCardData is updated
       ...
       *
       * WHAT IT DOES:
       * 1. Monitors chatCardData, setApiStatus for changes
       * 2. Executes setLogMessage, setApiStatus functions
       * 3. Runs side effect logic
       *
       * WHAT IT CALLS:
       * - setLogMessage() - Function call
       ...
       *
       * DATA FLOW:
       * Input: chatCardData, setApiStatus state/props
       * Processing: Calls setLogMessage, setApiStatus to process data
       * Output: Side effects executed, cleanup registered
       *
       * DEPENDENCIES:
       * - chatCardData: Triggers when chatCardData changes
       * - setApiStatus: Triggers when setApiStatus changes
       *
       */
    useEffect(() => {
        if (chatCardData) {
       ...
       *
       * WHAT IT DOES:
       * 1. Monitors isError, setApiStatus for changes
       * 2. Executes console.error, setLogMessage, setApiStatus functions
       * 3. Runs side effect logic
       *
       * WHAT IT CALLS:
       * - console.error() - Function call
       ...
       *
       * DATA FLOW:
       * Input: isError, setApiStatus state/props
       * Processing: Calls console.error, setLogMessage, setApiStatus to process data
       * Output: Side effects executed, cleanup registered
       *
       * DEPENDENCIES:
