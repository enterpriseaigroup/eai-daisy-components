...
interface Props {
  ...
  /**
   * BUSINESS LOGIC: DocumentDownloadPanel
   *
   * WHY THIS EXISTS:
   * - Implements business logic requirement
   *
   * WHAT IT DOES:
   * 1. Implements DocumentDownloadPanel logic
   * 2. Calls helper functions: useFetchUploadedDocuments, useAuthStore, useDocDownload, useState, useProfileStore, useMemo, documents.filter, parseFloat, useEffect, fetchUploadedDocuments.mutate, useEffect, safeUpdateUserConfig, filteredDocuments.map, .toLocaleDateString, setAcceptRenames, goBackToStage
   * 3. Returns computed result
   *
   * WHAT IT CALLS:
   * - useFetchUploadedDocuments() - Function call
   * - useAuthStore() - Function call
   * - useDocDownload() - Function call
   ...
   * - filteredDocuments.map() - Function call
   * - .toLocaleDateString() - Function call
   * - setAcceptRenames() - Function call
   * - goBackToStage() - Function call
   *
   * WHY IT CALLS THEM:
   * - useFetchUploadedDocuments: Data fetching
   * - useAuthStore: Required functionality
   * - useDocDownload: Required functionality
   ...
   * - filteredDocuments.map: Required functionality
   * - .toLocaleDateString: Required functionality
   * - setAcceptRenames: State update
   * - goBackToStage: Required functionality
   *
   * DATA FLOW:
   * Input: Component state and props
   * Processing: Calls useFetchUploadedDocuments, useAuthStore, useDocDownload to process data
...
export default function DocumentDownloadPanel({
  ...
  // ✅ NEW: Filter out documents with confidence score of 0 aka not validated documents #1331
    /**
     * BUSINESS LOGIC: filteredDocuments
     *
     * WHY THIS EXISTS:
     * - Implements business logic requirement
     *
     ...
     *
     * DATA FLOW:
     * Input: documents state/props
     * Processing: Calls documents.filter, parseFloat to process data
     * Output: Computed value or side effect
     *
     * DEPENDENCIES:
  ...
  const filteredDocuments = useMemo(() => {
     ...
     *
     * WHAT IT DOES:
     * 1. Monitors accessToken for changes
     * 2. Executes fetchUploadedDocuments.mutate functions
     ...
     *
     * DATA FLOW:
     * Input: accessToken state/props
     * Processing: Calls fetchUploadedDocuments.mutate to process data
  ...
  useEffect(() => {
     ...
     *
     * WHAT IT DOES:
     * 1. Monitors documents, profileData, updateUserConfig for changes
     * 2. Executes safeUpdateUserConfig functions
     * 3. Runs side effect logic
     *
     * WHAT IT CALLS:
     * - safeUpdateUserConfig() - Function call
     ...
     *
     * DATA FLOW:
     * Input: documents, profileData, updateUserConfig state/props
     * Processing: Calls safeUpdateUserConfig to process data
     * Output: Side effects executed, cleanup registered
     *
     * DEPENDENCIES:
     * - documents: Triggers when documents changes
