...
// Utility functions
  /**
   * BUSINESS LOGIC: truncateText
   *
   * WHY THIS EXISTS:
   * - Implements business logic requirement
   *
   * WHAT IT DOES:
   * 1. Implements truncateText logic
   * 2. Calls helper functions: text.substring
   * 3. Returns computed result
   *
   * WHAT IT CALLS:
   * - text.substring() - Function call
   *
   * WHY IT CALLS THEM:
   * - text.substring: Required functionality
   *
   * DATA FLOW:
   * Input: Component state and props
   * Processing: Calls text.substring to process data
...
const truncateText = (text: string, maxLength: number): string => {
  ...
  /**
   * BUSINESS LOGIC: sanitizeDocumentType
   *
   * WHY THIS EXISTS:
   * - Implements business logic requirement
   *
   * WHAT IT DOES:
   * 1. Implements sanitizeDocumentType logic
   * 2. Calls helper functions: .replace, docType.toLowerCase
   * 3. Returns computed result
   *
   * WHAT IT CALLS:
   * - .replace() - Function call
   * - docType.toLowerCase() - Function call
   *
   * WHY IT CALLS THEM:
   * - .replace: Required functionality
   * - docType.toLowerCase: Required functionality
   *
   * DATA FLOW:
   * Input: Component state and props
   * Processing: Calls .replace, docType.toLowerCase to process data
   * Output: Computed value or side effect
   *
   */
const sanitizeDocumentType = (docType: string): string => {
  ...
  /**
   * BUSINESS LOGIC: getStatusStyles
   *
   * WHY THIS EXISTS:
   * - Implements business logic requirement
   *
   * WHAT IT DOES:
   * 1. Implements getStatusStyles logic
   * 2. Calls helper functions: status.toLowerCase
   * 3. Returns computed result
   *
   * WHAT IT CALLS:
   * - status.toLowerCase() - Function call
   *
   * WHY IT CALLS THEM:
   * - status.toLowerCase: Required functionality
   *
   * DATA FLOW:
   * Input: Component state and props
   * Processing: Calls status.toLowerCase to process data
   * Output: Computed value or side effect
   *
   */
...
// Custom hook for document operations
  /**
   * BUSINESS LOGIC: useDocumentOperations
   *
   * WHY THIS EXISTS:
   * - Implements business logic requirement
   *
   * WHAT IT DOES:
   * 1. Implements useDocumentOperations logic
   * 2. Calls helper functions: useState, useState, useState, useEffect, .forEach, setExpandedRows, Object.entries, useCallback, setExpandedRows, newExpanded.add, newExpanded.delete, newExpanded.has, useCallback, setMenuOpen, useCallback, alert, handleUploadClick, setModalOpen, setExpandedRows, setMenuOpen, useCallback, handleUploadClick, setExpandedRows, setMenuOpen
   * 3. Returns computed result
   *
   * WHAT IT CALLS:
   ...
   * - setExpandedRows() - Function call
   * - Object.entries() - Function call
   * - useCallback() - Function call
   * - setExpandedRows() - Function call
   * - newExpanded.add() - Function call
   * - newExpanded.delete() - Function call
   * - newExpanded.has() - Function call
   * - useCallback() - Function call
   * - setMenuOpen() - Function call
   * - useCallback() - Function call
   * - alert() - Function call
   * - handleUploadClick() - Function call
   * - setModalOpen() - Function call
   * - setExpandedRows() - Function call
   * - setMenuOpen() - Function call
   * - useCallback() - Function call
   * - handleUploadClick() - Function call
   ...
   * - setExpandedRows: State update
   * - Object.entries: Required functionality
   * - useCallback: Required functionality
   * - setExpandedRows: State update
   * - newExpanded.add: Required functionality
   * - newExpanded.delete: Required functionality
   * - newExpanded.has: Required functionality
   * - useCallback: Required functionality
   * - setMenuOpen: State update
   * - useCallback: Required functionality
   * - alert: Required functionality
   * - handleUploadClick: Required functionality
   * - setModalOpen: State update
   * - setExpandedRows: State update
   * - setMenuOpen: State update
   * - useCallback: Required functionality
   * - handleUploadClick: Required functionality
...
const useDocumentOperations = (
  ...
  // Auto-expand rows when files are uploaded
     ...
     *
     * WHAT IT DOES:
     * 1. Monitors uploadStatuses for changes
     * 2. Executes .forEach, setExpandedRows, Object.entries functions
     ...
     *
     * DATA FLOW:
     * Input: uploadStatuses state/props
     * Processing: Calls .forEach, setExpandedRows, Object.entries to process data
  ...
  useEffect(() => {
     ...
     * 2. Calls setExpandedRows, newExpanded.add, newExpanded.delete, newExpanded.has functions
     ...
     * DATA FLOW:
     * Input: Component state and props
     * Processing: Calls setExpandedRows, newExpanded.add, newExpanded.delete to process data
  ...
  const toggleRow = useCallback((docId: string) => {
    ...
    /**
     * BUSINESS LOGIC: handleMenuClick
     *
     * WHY THIS EXISTS:
     * - Implements business logic requirement
     *
     ...
     * DATA FLOW:
     * Input: Component state and props
     * Processing: Calls setMenuOpen to process data
     * Output: Event handled, state updated
     *
     * SPECIAL BEHAVIOR:
     * - Memoized for performance optimization
  ...
  const handleMenuClick = useCallback((docId: string, isChild: boolean, fileIndex?: number) => {
     ...
     * WHAT IT DOES:
     * 1. Handles user interaction or event
     * 2. Calls alert, handleUploadClick, setModalOpen, setExpandedRows, setMenuOpen functions
     * 3. Updates state or triggers effects
     *
     * WHAT IT CALLS:
     * - alert() - Function call
     ...
     *
     * DATA FLOW:
     * Input: uploadStatuses, handleUploadClick state/props
     * Processing: Calls alert, handleUploadClick, setModalOpen to process data
     * Output: Event handled, state updated
     *
     * DEPENDENCIES:
     * - uploadStatuses: Triggers when uploadStatuses changes
     * - handleUploadClick: Triggers when handleUploadClick changes
     *
     * SPECIAL BEHAVIOR:
     * - Memoized for performance optimization
     *
  ...
  const handleAddFile = useCallback((doc: Document, isMobile: boolean) => {
    ...
    /**
     * BUSINESS LOGIC: handleReplaceFileClick
     *
     * WHY THIS EXISTS:
     * - Implements business logic requirement
     *
     * WHAT IT DOES:
     * 1. Handles user interaction or event
     * 2. Calls handleUploadClick, setExpandedRows, setMenuOpen functions
     * 3. Updates state or triggers effects
     *
     * WHAT IT CALLS:
     * - handleUploadClick() - Function call
     ...
     *
     * DATA FLOW:
     * Input: handleUploadClick state/props
     * Processing: Calls handleUploadClick, setExpandedRows, setMenuOpen to process data
     * Output: Event handled, state updated
     *
     * DEPENDENCIES:
     * - handleUploadClick: Triggers when handleUploadClick changes
...
// Main component
  /**
   * BUSINESS LOGIC: DocumentTable
   *
   * WHY THIS EXISTS:
   * - Implements business logic requirement
   *
   * WHAT IT DOES:
   * 1. Implements DocumentTable logic
   * 2. Calls helper functions: useDocumentTable, useDocumentOperations, useRef, useMemo, propDocuments.map, statuses.some, useCallback, statuses.some, statuses.some, statuses.every, statuses.some, useCallback, statuses.some, statuses.some, statuses.filter, useEffect, setMenuOpen, .contains, document.addEventListener, document.removeEventListener, useMemo, documents.every, documents.map, expandedRows.has, .some, .flat, Object.values, .some, .flat, Object.values, getSummaryStatus, getSummaryDescription, toggleRow, truncateText, .trim, .trim, handleAddFile, docUploadStatuses.map, documents.map, getSummaryStatus, sanitizeDocumentType, .trim, .trim, docUploadStatuses.map, setModalOpen, handleAddFile, getStatusStyles, .trim, .trim, docUploadStatuses.map, handleUploadClick, setModalOpen
   * 3. Returns computed result
   *
   * WHAT IT CALLS:
   * - useDocumentTable() - Function call
   * - useDocumentOperations() - Function call
   * - useRef() - Function call
   * - useMemo() - Function call
   * - propDocuments.map() - Function call
   * - statuses.some() - Function call
   * - useCallback() - Function call
   * - statuses.some() - Function call
   * - statuses.some() - Function call
   * - statuses.every() - Function call
   * - statuses.some() - Function call
   * - useCallback() - Function call
   * - statuses.some() - Function call
   * - statuses.some() - Function call
   * - statuses.filter() - Function call
   * - useEffect() - Function call
   * - setMenuOpen() - Function call
   ...
   * - useMemo() - Function call
   * - documents.every() - Function call
   * - documents.map() - Function call
   * - expandedRows.has() - Function call
   * - .some() - Function call
   * - .flat() - Function call
   ...
   * - .flat() - Function call
   * - Object.values() - Function call
   * - getSummaryStatus() - Function call
   * - getSummaryDescription() - Function call
   * - toggleRow() - Function call
   * - truncateText() - Function call
   * - .trim() - Function call
   * - .trim() - Function call
   * - handleAddFile() - Function call
   * - docUploadStatuses.map() - Function call
   * - documents.map() - Function call
   * - getSummaryStatus() - Function call
   * - sanitizeDocumentType() - Function call
   * - .trim() - Function call
   * - .trim() - Function call
   * - docUploadStatuses.map() - Function call
   * - setModalOpen() - Function call
   * - handleAddFile() - Function call
   * - getStatusStyles() - Function call
   * - .trim() - Function call
   * - .trim() - Function call
   * - docUploadStatuses.map() - Function call
   * - handleUploadClick() - Function call
   ...
   *
   * WHY IT CALLS THEM:
   * - useDocumentTable: Required functionality
   * - useDocumentOperations: Required functionality
   * - useRef: Required functionality
   * - useMemo: Required functionality
   * - propDocuments.map: Required functionality
   * - statuses.some: Required functionality
   * - useCallback: Required functionality
   * - statuses.some: Required functionality
   * - statuses.some: Required functionality
   * - statuses.every: Required functionality
   * - statuses.some: Required functionality
   * - useCallback: Required functionality
   * - statuses.some: Required functionality
   * - statuses.some: Required functionality
   * - statuses.filter: Required functionality
   * - useEffect: Required functionality
   * - setMenuOpen: State update
   ...
   * - useMemo: Required functionality
   * - documents.every: Required functionality
   * - documents.map: Required functionality
   * - expandedRows.has: Required functionality
   * - .some: Required functionality
   * - .flat: Required functionality
   * - Object.values: Required functionality
   * - .some: Required functionality
   * - .flat: Required functionality
   * - Object.values: Required functionality
   * - getSummaryStatus: Required functionality
   * - getSummaryDescription: Required functionality
   * - toggleRow: Required functionality
   * - truncateText: Required functionality
   * - .trim: Required functionality
   * - .trim: Required functionality
   * - handleAddFile: Required functionality
   * - docUploadStatuses.map: Required functionality
   * - documents.map: Required functionality
   * - getSummaryStatus: Required functionality
   * - sanitizeDocumentType: Required functionality
   * - .trim: Required functionality
   * - .trim: Required functionality
   * - docUploadStatuses.map: Required functionality
   * - setModalOpen: State update
   * - handleAddFile: Required functionality
   * - getStatusStyles: Required functionality
   * - .trim: Required functionality
   * - .trim: Required functionality
   * - docUploadStatuses.map: Required functionality
   * - handleUploadClick: Required functionality
   ...
   * DATA FLOW:
   * Input: Component state and props
   * Processing: Calls useDocumentTable, useDocumentOperations, useRef to process data
   * Output: Computed value or side effect
   *
   */
const DocumentTable: React.FC<DocumentTableProps> = ({ 
  ...
  // Memoize processed documents
     ...
     *
     * DATA FLOW:
     * Input: propDocuments, uploadStatuses state/props
     * Processing: Calls propDocuments.map, statuses.some to process data
     * Output: Computed value or side effect
     *
     * DEPENDENCIES:
     * - propDocuments: Triggers when propDocuments changes
     * - uploadStatuses: Triggers when uploadStatuses changes
     *
     * SPECIAL BEHAVIOR:
     * - Memoized for performance optimization
  ...
  // Status calculation functions
    /**
     * BUSINESS LOGIC: getSummaryStatus
     *
     * WHY THIS EXISTS:
     * - Implements business logic requirement
     *
     * WHAT IT DOES:
     * 1. Handles user interaction or event
     * 2. Calls statuses.some, statuses.some, statuses.every, statuses.some functions
     * 3. Updates state or triggers effects
     *
     * WHAT IT CALLS:
     * - statuses.some() - Function call
     ...
     * DATA FLOW:
     * Input: Component state and props
     * Processing: Calls statuses.some, statuses.some, statuses.every to process data
     * Output: Event handled, state updated
     *
     * SPECIAL BEHAVIOR:
     * - Memoized for performance optimization
  ...
  const getSummaryStatus = useCallback((doc: Document, statuses: UploadStatus[]): string => {
    ...
    /**
     * BUSINESS LOGIC: getSummaryDescription
     *
     * WHY THIS EXISTS:
     * - Implements business logic requirement
     *
     * WHAT IT DOES:
     * 1. Handles user interaction or event
     * 2. Calls statuses.some, statuses.some, statuses.filter functions
     * 3. Updates state or triggers effects
     *
     * WHAT IT CALLS:
     * - statuses.some() - Function call
     ...
     * DATA FLOW:
     * Input: Component state and props
     * Processing: Calls statuses.some, statuses.some, statuses.filter to process data
     * Output: Event handled, state updated
     *
     * SPECIAL BEHAVIOR:
     * - Memoized for performance optimization
  ...
  // Click outside handler
     ...
     *
     * WHAT IT DOES:
     * 1. Monitors setMenuOpen for changes
     * 2. Executes setMenuOpen, .contains, document.addEventListener, document.removeEventListener functions
     * 3. Runs side effect logic
     *
     * WHAT IT CALLS:
     * - setMenuOpen() - Function call
     ...
     *
     * DATA FLOW:
     * Input: setMenuOpen state/props
     * Processing: Calls setMenuOpen, .contains, document.addEventListener to process data
     * Output: Side effects executed, cleanup registered
     *
     * DEPENDENCIES:
     * - setMenuOpen: Triggers when setMenuOpen changes
     *
     */
  useEffect(() => {
      /**
       * BUSINESS LOGIC: handleClickOutside
       *
       * WHY THIS EXISTS:
       * - Implements business logic requirement
       *
       * WHAT IT DOES:
       * 1. Implements handleClickOutside logic
       * 2. Calls helper functions: setMenuOpen, .contains
       * 3. Returns computed result
       *
       * WHAT IT CALLS:
       * - setMenuOpen() - Function call
       ...
       * DATA FLOW:
       * Input: Component state and props
       * Processing: Calls setMenuOpen, .contains to process data
       * Output: Computed value or side effect
       *
       */
  ...
  // Check if all documents are empty
     ...
     *
     * DATA FLOW:
     * Input: documents, uploadStatuses state/props
     * Processing: Calls documents.every to process data
     * Output: Computed value or side effect
     *
     * DEPENDENCIES:
