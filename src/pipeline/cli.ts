...
export class ConsoleFormatter {
  ...
  results(result: PipelineResult): void {
    if (this.quiet && result.success) return;
    ...
    if (result.success) {
      ...
      if (result.progress.stats.componentsDiscovered > 0) {
        this.table(
          ['Metric', 'Count'],
          [
            ['Components Discovered', result.progress.stats.componentsDiscovered.toString()],
            ['Components Parsed', result.progress.stats.componentsParsed.toString()],
            ['Components Analyzed', result.progress.stats.componentsAnalyzed.toString()],
            ['Errors Encountered', result.progress.stats.errorsEncountered.toString()],
            ['Warnings Generated', result.progress.stats.warningsGenerated.toString()]
          ]
        );
      }
  ...
  private getPhaseColor(phase: string): (text: string) => string {
    switch (phase) {
      case 'discovery': return chalk.blue;
      case 'parsing': return chalk.yellow;
      case 'dependency-analysis': return chalk.magenta;
      case 'inventory-generation': return chalk.cyan;
...
export class CLIApplication {
   ...
   */
  private async handleConfigCommand(options: { config?: string }): Promise<void> {
    ...
    try {
      ...
      this.formatter.error(error instanceof Error ? error.message : 'Unknown error');
    }
  }
   ...
   * Convert CLI options to pipeline options
   */
  private convertToPipelineOptions(options: Partial<CLIOptions>): PipelineOptions {
    return {
  ...
  private async runPipeline(
    ...
    const eventHandler: PipelineEventHandler = {
      ...
      onComponentProcessed: (path, result) => {
        this.formatter.debug(`Processed: ${basename(path)} - ${result.success ? 'Success' : 'Failed'}`);
      }
    };
  ...
  private async handleOutput(
    ...
    if (options.json !== false && result.success) {
      const jsonPath = resolve(options.output || './output', 'pipeline-result.json');
  ...
  private showBanner(): void {
    ...
    console.log(chalk.cyan('║') + chalk.white('                 DAISY EXTRACT                        ') + chalk.cyan('║'));
    console.log(chalk.cyan('║') + chalk.gray('         Component Extraction Pipeline                ') + chalk.cyan('║'));
    console.log(chalk.cyan('║') + chalk.gray('       Transform DAISY v1 to Configurator v2         ') + chalk.cyan('║'));
    console.log(chalk.cyan('╚══════════════════════════════════════════════════════════╝'));
    console.log('');
  }
}
